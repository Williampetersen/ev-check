...
import { saveWeeklySchedule } from "@/lib/server/dashboard";
...
export async function POST(request: Request) {
  ...
  try {
    await saveWeeklySchedule(await request.formData());
    return NextResponse.redirect(new URL("/admin?view=settings&saved=schedule", request.url), 303);
  } catch {
